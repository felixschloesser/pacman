    ...
    do {
        ...
        switch(choice) {
            case '1': clear ();
                ...
                mvprintw(row/2,(col-4-strlen(mesg))/2,"%s",mesg);
                getstr(playerName);
                mvprintw(row/2+2, (col-42-strlen(mesg)), "Press any button to start the game.");
            ...
            case '2': highscoreWin = newwin(11, 28, 9, 25);
                ...
                char* nameList = (char*)malloc(100*sizeof(char));
                int i = 0;
                do{
                    bkgd(COLOR_PAIR(1));
                    fgets(nameList, 1000*sizeof(char), data);
                    fscanf(data, "%d", &numberArray[i]);
                    
                
                    
                    mvwprintw(highscoreWin, i+1, 0," %s\t%d\tCookies ",nameList, 195-numberArray[i]);
